...
# INFO: You need to combine data ntuples from all years by running and generate workspaes
y = 'combined'
wsdir = '/eos/user/b/bjoshi/VHAnomalous/workspaces'
wsdir = '{wsdir}/{tag}/combined'.format(tag=tag, wsdir=wsdir)
lumi_ = lumiMap[y]
for cat_ in CATEGORIES:
    cmd = 'bash runBackgroundScripts.sh -i {wsdir}/allData.root -p none -f {cat} --ext {tag}_{year}_{ext} --catOffset 0 --intLumi {lumi} --year {year} --batch local --queue cmsan --sigFile none --isData --fTest'.format(year=y, tag=tag, ext=ext, wsdir=wsdir, cat=cat_, lumi=lumi_)
    os.system(cmd)
