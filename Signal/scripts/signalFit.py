...
def get_options():
  ...
  # Parameter values
  parser.add_option('--replacementThreshold', dest='replacementThreshold', default=45, type='int', help="Nevent threshold to trigger replacement dataset")
...
def signalFit(opt):
  ...
  # Load RooRealVars
  inputFile = "%s/output*M%s*%s.root"%(opt.inputWSDir,MHNominal,opt.proc)
  print(inputFile)
  nominalWSFileName = glob.glob("%s/output*%s*.root"%(opt.inputWSDir,MHNominal))[0]
  
  #inputFile = "%s/allSignal.root"%(opt.inputWSDir)
  #nominalWSFileName = glob.glob(inputFile)[0]
  ...
  if opt.skipZeroes:
    ...
    print("Filename: %s" % inputFile)
    #WSFileName = glob.glob("%s/output*%s*.root"%(opt.inputWSDir,MHNominal))[0]
    WSFileName = glob.glob(inputFile)[0]
    print("WSFileName: %s" % WSFileName)
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
    print("%s_%s_%s_%s"%(procToData(opt.proc),MHNominal,sqrts__,opt.cat))
  ...
  for mp in opt.massPoints.split(","):
    if 'ALT' in procRVFit and '0PM' not in procRVFit and mp!=MHNominal: continue
  ...
  if( datasetRVForFit[MHNominal].numEntries() < opt.replacementThreshold  )|( datasetRVForFit[MHNominal].sumEntries() < 0. ):
    ...
    for mp in opt.massPoints.split(","):
      ...
      f = ROOT.TFile(WSFileName,"read")
      inputWS = f.Get(inputWSName__)
      print("%s_%s_%s_%s"%(procToData(procReplacementFit),mp,sqrts__,catReplacementFit))
  ...
  if not opt.skipVertexScenarioSplit:
    datasetWVForFit = od()
    for mp in opt.massPoints.split(","):
      if 'ALT' in procWVFit and '0PM' not in procWVFit and mp!=MHNominal: continue
    ...
    if( datasetWVForFit[MHNominal].numEntries() < opt.replacementThreshold  )|( datasetWVForFit[MHNominal].sumEntries() < 0. ):
      ...
      print('Replacement: ', procReplacementFit, catReplacementFit)
      for mp in opt.massPoints.split(","):
        print(procReplacementFit, catReplacementFit) 
        if 'ALT' in procReplacementFit and mp!=MHNominal: continue
        fname_ = "%s/output*%s*%s.root"%(opt.inputWSDir,mp,procReplacementFit)
        print('Replacement filename: %s'%fname_)
        WSFileName = glob.glob(fname_)[0]
        #WSFileName = glob.glob(inputFile)[0]
        f = ROOT.TFile(WSFileName,"read")
        inputWS = f.Get(inputWSName__)
        print("%s_%s_%s_%s"%(procToData(procReplacementFit),mp,sqrts__,catReplacementFit))
  ...
  if not opt.skipBeamspotReweigh:
    ...
    for mp,d in datasetRVForFit.iteritems(): 
      if 'ALT' in d and '0PM' not in d and mp!=MHNominal: continue
  ...
  if not opt.useDCB:
    ...
    nRV = int(ngauss["%s_125_13TeV_%s"%(procRVFit,catRVFit)]['nRV'])
    if opt.skipVertexScenarioSplit: print " --> Fitting function: convolution of nGaussians (%g)"%nRV
      ...
      nWV = int(ngauss["%s_125_13TeV_%s"%(procWVFit,catWVFit)]['nWV'])
