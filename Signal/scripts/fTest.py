...
def fTest(opt):
  ...
  for proc in opt.procs.split(","):
    ...
    dname_ = "%s_%s_%s_%s"%(procToData(proc),opt.mass,sqrts__,opt.cat)
    #print(dname_)
    d = None
    try:
      d = reduceDataset(inputWS.data(dname_),aset)
    except:
      print('RooDataset: %s not found in %s'%(dname_, WSDir))
  ...
  with open("%s/outdir_%s/fTest/json/nGauss_%s.json"%(swd__,opt.ext,opt.cat),"w") as ff:
    ff.write("{\n")
    # Iterate over rows in dataframe: sorted by sumEntries
    pitr = 1
    for ir,r in df.sort_values('sumEntries',ascending=False).iterrows():
      k = "\"%s_%s_13TeV_%s\""%(r['proc'],opt.mass,opt.cat)
